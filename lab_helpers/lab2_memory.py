...
def create_or_get_memory_resource():
    try:
        ...
        print(f"✅ Using existing memory from SSM: {memory_id}")
        return memory_id
    except:
        try:
            strategies = [
                {
                    StrategyType.USER_PREFERENCE.value: {
                        "name": "ProductManagerPreferences",
                        "description": "Captures customer preferences and behavior",
                        "namespaces": ["product/manager/{actorId}/preferences"],
                    }
                },
                {
                    StrategyType.SEMANTIC.value: {
                        ...
                        "namespaces": ["product/manager/{actorId}/semantic"],
                    }
                },
            ]
            ...
            # If memory already exists, try to find it
            if "already exists" in str(e):
                print("⚠️ Memory already exists, retrieving existing memory...")
                try:
                    response = memory_client.gmcp_client.list_memories()
                    
                    memories = response.get('memories', [])
                    
                    # Memory ID starts with the memory name
                    for memory in memories:
                        mem_id = memory.get('id', memory.get('memoryId', ''))
                        if mem_id.startswith(memory_name):
                            print(f"✅ Found existing memory: {mem_id}")
                            try:
                                put_ssm_parameter("/app/productlaunch/agentcore/memory_id", mem_id)
                            except:
                                pass
                            return mem_id
                    
                    print(f"⚠️ Could not find memory starting with '{memory_name}' in list")
                except Exception as list_error:
                    print(f"Error listing memories: {list_error}")
            
            print(f"❌ Failed to create or retrieve memory: {e}")
            raise Exception(f"Could not create or retrieve memory: {e}")
...
def delete_memory(memory_hook):
    try:
        ...
        ssm_client.delete_parameter(Name="/app/productlaunch/agentcore/memory_id")
    except Exception:
        pass
