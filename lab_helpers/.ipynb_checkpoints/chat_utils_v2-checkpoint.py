...
def get_cognito_client_secret() -> str:
    ...
    response = client.describe_user_pool_client(
        UserPoolId=get_ssm_parameter("/app/productlaunch/agentcore/userpool_id"),
        ClientId=get_ssm_parameter("/app/productlaunch/agentcore/machine_client_id"),
    )
...
def extract_image_paths(text):
    """Extract image and PDF paths from agent response."""
    image_pattern = r'/materials/marketing_image_\d+_\d+\.png'
    pdf_pattern = r'/materials/marketing_poster_\d+_\d+\.pdf'
    
    # Look for base64 in JSON format or special markers
    base64_json_pattern = r'"image_base64":\s*"([A-Za-z0-9+/=]+)"'
    base64_marker_pattern = r'\[IMAGE_DATA\]([A-Za-z0-9+/=]+)\[/IMAGE_DATA\]'
    
    images = re.findall(image_pattern, text)
    pdfs = re.findall(pdf_pattern, text)
    base64_images = re.findall(base64_json_pattern, text) + re.findall(base64_marker_pattern, text)
    
    return {
        'images': images,
        'pdfs': pdfs,
        'base64_images': base64_images,
        'has_media': len(images) > 0 or len(pdfs) > 0 or len(base64_images) > 0
    }
