...
import json
import time
from chat_utils import make_urls_clickable, extract_image_paths
import os
import sys
...
from utils import get_ssm_parameter, get_product_launch_secret

secret = get_product_launch_secret()
...
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        ...
        # Display marketing materials if present in message
        if message["role"] == "assistant":
            media_info = extract_image_paths(message["content"])
            if media_info['has_media']:
                for image_path in media_info['images']:
                    actual_path = image_path.replace('/materials/', 'generated_materials/')
                    if os.path.exists(actual_path):
                        st.image(actual_path, caption="Marketing Poster", use_container_width=True)
...
if prompt := st.chat_input("What is up?"):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            message_placeholder.markdown(f'<span class="thinking-bubble">🤖 💭 Product Launch Agent is thinking...</span>', unsafe_allow_html=True)
            ...
            media_info = extract_image_paths(answer)
            ...
            # Display marketing materials if found
            if media_info['has_media']:
                st.markdown("---")
                st.markdown("### 🎨 Marketing Materials Generated")
                
                for image_path in media_info['images']:
                    # Convert /materials/ path to actual file path
                    actual_path = image_path.replace('/materials/', 'generated_materials/')
                    if os.path.exists(actual_path):
                        st.image(actual_path, caption="Marketing Poster Image", use_container_width=True)
                    else:
                        st.warning(f"Image not found: {actual_path}")
                
                for pdf_path in media_info['pdfs']:
                    actual_path = pdf_path.replace('/materials/', 'generated_materials/')
                    if os.path.exists(actual_path):
                        with open(actual_path, 'rb') as pdf_file:
                            st.download_button(
                                label="📄 Download Marketing Poster PDF",
                                data=pdf_file,
