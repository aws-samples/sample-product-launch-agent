...
username = "testuser"
secret_name = "product_launch_agent"

role_name = f"ProductLaunchAgentBedrockAgentCoreRole-{REGION}"
policy_name = f"ProductLaunchAgentBedrockAgentCorePolicy-{REGION}"
...
def get_cognito_client_secret() -> str:
    ...
    response = client.describe_user_pool_client(
        UserPoolId=get_ssm_parameter("/app/productlaunch/agentcore/pool_id"),
        ClientId=get_ssm_parameter("/app/productlaunch/agentcore/client_id"),
    )
...
def save_product_launch_secret(secret_value):
    ...
    try:
        secrets_client.create_secret(
            ...
            Description="Secret containing the Cognito Configuration for the Product Launch Agent",
        )
...
def delete_product_launch_secret():
...
def get_or_create_cognito_pool(refresh_token=False):
    ...
    try:
        ...
        cognito_config_str = get_product_launch_secret()
    ...
    try:
        ...
        put_ssm_parameter("/app/productlaunch/agentcore/client_id", client_id)
        put_ssm_parameter("/app/productlaunch/agentcore/pool_id", pool_id)
        put_ssm_parameter(
            "/app/productlaunch/agentcore/cognito_discovery_url", discovery_url
        )
        put_ssm_parameter("/app/productlaunch/agentcore/client_secret", client_secret)

        save_product_launch_secret(json.dumps(cognito_config))
...
def create_agentcore_runtime_execution_role():
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": [
                    ...
                    f"arn:aws:bedrock-agentcore:{region}:{account_id}:workload-identity-directory/default/workload-identity/product_launch_agent-*",
                ],
            },
            {
    ...
    try:
        ...
        put_ssm_parameter(
            "/app/productlaunch/agentcore/runtime_execution_role_arn",
...
def runtime_resource_cleanup(runtime_arn: str = None):
    try:
        ...
        for repo in repositories["repositories"]:
            if "bedrock-agentcore-product_launch_agent" in repo["repositoryName"]:
